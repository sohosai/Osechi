...
- **機能と流れ**:
  1. **Bump Version ジョブ**:
     - cargo-editを使って Cargo.toml のバージョン指定を自動で書き換えます。
  ...
  2. **Build Release ジョブ**:
     ...
     - 生成された実行ファイルは各OSごとにリネームされ、GitHub Actionsの Artifacts（アーティファクト）としてアップロードされます。
  3. **Create GitHub Release ジョブ**:
     - 生成されたArtifactsをダウンロードし、GitHub Releases を自動で作成します。
     - 前回のリリース以降にマージされたPRの一覧リリースノートも自動生成します。
     - そのリリースに各OS用バイナリを添付します。