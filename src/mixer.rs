...
/// 信号のピーク振幅(線形, 0.0-1.0)を、`gain_to_db`と同じ-60dB〜0dBの
/// スケールでメーター表示用の位置(0.0-1.0)に変換する。
///
/// メーターの`level`は本来リニアな振幅値だが、そのままバンド数に線形分割
/// すると-40dBを下回るような静かな信号がほぼ点灯しない。フェーダーと同じ
/// dBスケールに揃えることで、小さい信号でも視覚的に反応が分かるようにする。
pub fn peak_to_meter_position(peak: f32) -> f32 {
    if peak <= 0.0 {
        return 0.0;
    }
